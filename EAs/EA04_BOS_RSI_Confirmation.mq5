...
input double InpBuyRSI=55;
input double InpSellRSI=45;
...
void OnTick(){
   ...
   double c=r[1].close, atr=iATRValue(InpATRPeriod,1), rv=RSI(InpRSIPeriod,1), buffer=InpBreakBufferPts*_Point;
   if(c>ph1+buffer && rv>=InpBuyRSI){
      double sl=pl1-0.3*atr, tp=c+(c-sl)*InpTP_RR; OpenBuy(sl,tp);
   } else if(c<pl1-buffer && rv<=InpSellRSI){
