...
void OnTick(){
   ...
   MqlRates r[]; ArraySetAsSeries(r,true); int n=InpStructureLookback+60; if(CopyRates(_Symbol,_Period,0,n,r)<n) return;
   double ph1,ph2,pl1,pl2; if(!GetLastTwoPivots(ph1,ph2,pl1,pl2)) return;
   double fast=SMA(InpFastMA,1), slow=SMA(InpSlowMA,1), c=r[1].close, atr=iATRValue(InpATRPeriod,1), buffer=InpBreakBufferPts*_Point;
   if(c>ph1+buffer && fast>slow){
      double sl=pl1-0.3*atr, tp=c+(c-sl)*InpTP_RR; OpenBuy(sl,tp);
   } else if(c<pl1-buffer && fast<slow){
      double sl=ph1+0.3*atr, tp=c-(sl-c)*InpTP_RR; OpenSell(sl,tp);
   }
}
