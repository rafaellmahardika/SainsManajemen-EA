...
int GetTrendTF(ENUM_TIMEFRAMES tf){
   MqlRates r[]; ArraySetAsSeries(r,true);
   int need=InpHTFLookback+20; if(CopyRates(_Symbol,tf,0,need,r)<need) return 0;
   double h1=0,h2=0,l1=0,l2=0; int hc=0,lc=0;
   for(int i=InpPivotRight+1;i<InpHTFLookback;i++){
      if(hc<2&&IsPivotHigh(r,i,InpPivotLeft,InpPivotRight)){ if(hc==0) h1=r[i].high; else h2=r[i].high; hc++; }
      if(lc<2&&IsPivotLow(r,i,InpPivotLeft,InpPivotRight)){ if(lc==0) l1=r[i].low; else l2=r[i].low; lc++; }
      if(hc>=2&&lc>=2)break;
   }
   if(hc<2||lc<2)return 0;
   if(h1>h2&&l1>l2)return 1; if(h1<h2&&l1<l2)return -1; return 0;
}
...
void OnTick(){
   ...
   int htf=GetTrendTF(InpHTF); if(htf==0)return;
   MqlRates r[]; ArraySetAsSeries(r,true); if(CopyRates(_Symbol,InpLTF,0,InpStructureLookback+10,r)<InpStructureLookback+10)return;
   ...
   if(htf==1&&c>ph1+buffer){double sl=pl1-0.4*atr,tp=c+(c-sl)*InpTP_RR;OpenBuy(sl,tp);}
   else if(htf==-1&&c<pl1-buffer){double sl=ph1+0.4*atr,tp=c-(sl-c)*InpTP_RR;OpenSell(sl,tp);}
}
