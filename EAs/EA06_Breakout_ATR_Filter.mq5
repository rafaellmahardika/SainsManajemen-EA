...
input double InpMinATRPoints=100;
input double InpMaxATRPoints=1500;
...
void OnTick(){
   ...
   double atr=iATRValue(InpATRPeriod,1), atrPts=atr/_Point, c=r[1].close, buffer=InpBreakBufferPts*_Point;
   if(atrPts<InpMinATRPoints || atrPts>InpMaxATRPoints) return;
   if(c>ph1+buffer){ double sl=pl1-0.2*atr, tp=c+(c-sl)*InpTP_RR; OpenBuy(sl,tp); }
   else if(c<pl1-buffer){ double sl=ph1+0.2*atr, tp=c-(sl-c)*InpTP_RR; OpenSell(sl,tp); }
}
