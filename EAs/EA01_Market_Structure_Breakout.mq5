...
void OnTick(){
   ...
   int n=InpStructureLookback+10; if(CopyRates(_Symbol,_Period,0,n,r)<n) return;
   double ph1,ph2,pl1,pl2; if(!GetLastTwoPivots(ph1,ph2,pl1,pl2)) return;
   double buffer=InpBreakBufferPts*_Point, atr=iATRValue(InpATRPeriod,1);
   double c=r[1].close;
   if(c>ph1+buffer){
      double sl=pl1-0.2*atr;
      double tp=c+(c-sl)*InpTP_RR;
      OpenBuy(sl,tp);
   }
   else if(c<pl1-buffer){
      double sl=ph1+0.2*atr;
      double tp=c-(sl-c)*InpTP_RR;
      OpenSell(sl,tp);
   }
}
