...
void OnTick(){
   ...
   int trend=TrendState(); if(trend==0) return;
   MqlRates r[]; ArraySetAsSeries(r,true); if(CopyRates(_Symbol,_Period,0,5,r)<5) return;
   double atr=iATRValue(InpATRPeriod,1), c=r[1].close;
   double ph1,ph2,pl1,pl2; if(!GetLastTwoPivots(ph1,ph2,pl1,pl2)) return;
   if(trend==1 && c>pl1+InpMinSwingATR*atr*0.05){
      double sl=pl1-0.5*atr, tp=c+(c-sl)*InpTP_RR; OpenBuy(sl,tp);
   }
   if(trend==-1 && c<ph1-InpMinSwingATR*atr*0.05){
      double sl=ph1+0.5*atr, tp=c-(sl-c)*InpTP_RR; OpenSell(sl,tp);
   }
}
