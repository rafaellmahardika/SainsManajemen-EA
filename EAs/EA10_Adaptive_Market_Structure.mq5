...
input int InpTrendFastMA=20;
input int InpTrendSlowMA=100;
input int InpAdaptiveRSI=14;
input double InpADXLikeThreshold=0.0;
int OnInit(){ trade.SetExpertMagicNumber(InpMagic); trade.SetDeviationInPoints(InpSlippagePoints); return INIT_SUCCEEDED; }
double EfficiencyRatio(int period,int shift){
   MqlRates r[]; ArraySetAsSeries(r,true); if(CopyRates(_Symbol,_Period,0,period+shift+2,r)<period+shift+2)return 0;
   double net=MathAbs(r[shift].close-r[shift+period].close), sum=0;
   for(int i=shift;i<shift+period;i++)sum+=MathAbs(r[i].close-r[i+1].close);
...
void OnTick(){
   ...
   MqlRates r[]; ArraySetAsSeries(r,true); if(CopyRates(_Symbol,_Period,0,InpStructureLookback+110,r)<InpStructureLookback+110)return;
   double ph1,ph2,pl1,pl2; if(!GetLastTwoPivots(ph1,ph2,pl1,pl2))return;
   double fast=SMA(InpTrendFastMA,1),slow=SMA(InpTrendSlowMA,1),rv=RSI(InpAdaptiveRSI,1);
   double er=EfficiencyRatio(20,1),atr=iATRValue(InpATRPeriod,1),c=r[1].close,buffer=InpBreakBufferPts*_Point;
   // Adaptive rule: require both structural breakout and a sufficiently directional regime.
   double regime=0.50+0.50*er;
   if(c>ph1+buffer && fast>slow && rv>50 && regime>=0.60){
      double sl=pl1-(0.35+0.50*er)*atr; double tp=c+(c-sl)*(InpTP_RR+er); OpenBuy(sl,tp);
   }
   else if(c<pl1-buffer && fast<slow && rv<50 && regime>=0.60){
      double sl=ph1+(0.35+0.50*er)*atr; double tp=c-(sl-c)*(InpTP_RR+er); OpenSell(sl,tp);
   }
}
