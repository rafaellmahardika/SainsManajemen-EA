...
input double InpPullbackZoneATR=0.60;
...
void OnTick(){
   ...
   MqlRates r[]; ArraySetAsSeries(r,true); if(CopyRates(_Symbol,_Period,0,15,r)<15) return;
   double ph1,ph2,pl1,pl2; if(!GetLastTwoPivots(ph1,ph2,pl1,pl2)) return;
   int trend=TrendState(); double atr=iATRValue(InpATRPeriod,1), c=r[1].close;
   if(trend==1){
      double mid=(ph1+pl1)/2.0;
      if(MathAbs(c-mid)<=InpPullbackZoneATR*atr){
         if(c>r[2].high){ double sl=pl1-0.4*atr, tp=c+(c-sl)*InpTP_RR; OpenBuy(sl,tp); }
      }
   } else if(trend==-1){
      double mid=(ph1+pl1)/2.0;
      if(MathAbs(c-mid)<=InpPullbackZoneATR*atr){
         if(c<r[2].low){ double sl=ph1+0.4*atr, tp=c-(sl-c)*InpTP_RR; OpenSell(sl,tp); }
      }
   }
}
